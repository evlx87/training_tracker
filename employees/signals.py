...
def invalidate_training_report_cache(sender, instance, **kwargs):
    try:
        ...
        logger.error(
            ...
            sender.__name__, str(e), exc_info=True
        )
...
def log_user_login(sender, request, user, **kwargs):
    try:
        ...
        logger.info(
            ...
            user.username, request.path, request.method, post_data
        )
    except Exception as e:
        logger.error(
...
def log_user_login_failed(sender, credentials, request, **kwargs):
    try:
        ...
        logger.warning(
            ...
            username, request.path, request.method, post_data
        )
    except Exception as e:
        logger.error(
            'Ошибка при логировании неуспешной попытки входа пользователя %s: %s',
            username, str(e), exc_info=True
        )